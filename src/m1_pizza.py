...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(90.0, 50.0), rg.Point(74.9, 81.3), rg.Point(41.1, 89.0), rg.Point(14.0, 67.4),
                rg.Point(14.0, 32.6), rg.Point(41.1, 11.0), rg.Point(74.9, 18.7)]
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  12 teal dots.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 200), 160)
    draw_points_on_circle(window, circle, 12, 'teal')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    j = generate_points_on_circle(circle, number_of_points)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # -------------------------------------------------------------------------
    circle.attach_to(window)
    j = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(j)):
        f = rg.Line(circle.center, j[k])
        f.color = color
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = 'POLYGON tests 3:  9 segments with medium red lines.'
    window = rg.RoseWindow(550, 400, title)
    circle = rg.Circle(rg.Point(350, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 9, 'red', 5)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # -------------------------------------------------------------------------
    circle.attach_to(window)
    j = generate_points_on_circle(circle, number_of_segments)
    f = rg.Line(j[number_of_segments-1], j[0])
    f.color = color
    f.thickness = thickness
    f.attach_to(window)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        f = rg.Line(j[k], j[k + hops_to_next_point])
        f.color = color
        f.thickness = thickness
        f.attach_to(window)
    window.render()
